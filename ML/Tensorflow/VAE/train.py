...
def main():
    ...
    parser.add_argument('--learning_rate_vae', type=float, default=0.00001,
...
def train(args):
    ...
    dcganvae = CNNVAE(data_name="ImageNet", batch_size=batch_size, learning_rate=learning_rate,
                      learning_rate_vae=learning_rate_vae, beta1=beta1, keep_prob=keep_prob)
    counter = 0
    ...
    for epoch in range(training_epochs):
        ...
        for i in range(total_batch):
            if isMnist:
                ...
                batch_images = np.reshape(
                    batch_images, [batch_size, 28, 28, 1])
            else:
                batch_images = celeb.next_batch(batch_size)
                print batch_images.shape
                batch_images = np.reshape(
                    batch_images, [batch_size, 64, 64, 3])
            vae_loss = dcganvae.partial_train(batch_images)
        #   print d_loss
        #   print g_loss
        #   print vae_loss
        #   print i
        ...
        #   assert( g_loss < 1000000 ) # make sure it is not NaN or Inf
            if (counter + 1) % display_step == 0:
                print "Sample:", '%d' % ((i + 1) * batch_size), " Epoch:", '%d' % (epoch), "vae_loss=", "{:.4f}".format(vae_loss)
        ...
        if epoch >= 0:
            print "Epoch:", '%04d' % (epoch), "avg_vae_loss=", "{:.6f}".format(avg_vae_loss)
