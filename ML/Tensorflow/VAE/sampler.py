...
class Sampler():
  def __init__(self, data_name="Mnist"):
    self.mnist = None
    self.model = CNNVAE(data_name=data_name)
  ...
  def get_random_mnist(self, with_label = False):
    ...
    if with_label == True:
      data, label = self.mnist.train.next_batch(1, with_label)
      return data[0], label[0]
    return mnist.train.next_batch(1)[0]
  ...
  def encode(self, mnist_data):
    # new_shape = [1]+list(mnist_data.shape) + [1]
    return self.model.encode(mnist_data)
