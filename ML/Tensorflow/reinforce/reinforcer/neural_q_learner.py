...
class NeuralQLearner(object):
    ...
    def create_variables(self):
        ...
        # estimate rewards using the next state: r(s_t,a_t) + argmax_a
        # Q(s_{t+1}, a)
        with tf.name_scope("estimate_future_rewards"):
            ...
            if self.double_q_learning:
                ...
                self.action_selection = tf.argmax(tf.stop_gradient(
                    self.q_next_outputs), 1, name="action_selection")
                ...
                with tf.variable_scope("target_network"):
                    self.target_outputs = self.q_network(
                        self.next_states) * tf.cast(self.action_selection_mask, tf.float32)
                self.action_evaluation = tf.reduce_sum(
                    self.target_outputs, reduction_indices=[1, ])
                ...
                self.target_values = tf.reduce_max(self.next_action_scores, reduction_indices=[
        ...
        # loss of Q-network, target network is updated purely by q-network using value iteration
        # compute loss and norm-clipped gradients
        with tf.name_scope("compute_temporal_differences"):
            ...
            self.masked_action_score = tf.reduce_sum(
                self.action_scores * self.action_mask, reduction_indices=[1, ])
            self.temp_diff = self.masked_action_score - self.future_rewards
            ...
            self.reg_loss = self.reg_param * \
                tf.reduce_sum([tf.reduce_sum(tf.square(x))
                               for x in q_network_variables])
            # compute total loss and gradients
            self.loss = self.reg_loss + self.td_loss
        ...
        with tf.name_scope("update_target_network"):
            ...
            # update target network parameters using those of Q network
            q_network_variables = tf.get_collection(
                tf.GraphKeys.TRAINABLE_VARIABLES, scope="q_network")
    ...
    def eGreedyAction(self, state, explore=True):
        if explore and self.exploration > random.random():
            return random.randint(0, self.num_actions - 1)
        else:
            return self.session.run(self.predicted_actions, {self.states: state})[0]

    def annealExploration(self, strategy='linear'):
        ratio = max((self.anneal_steps - self.train_iteration) /
                    float(self.anneal_steps), 0)
        self.exploration = (self.init_exp - self.final_exp) * \
            ratio + self.final_exp
    ...
    def updateModel(self):
        ...
        for k, (s, a, r, ns, done) in enumerate(batch):
            states[k] = s
            rewards[k] = r
            action_mask[k][a] = 1
            if not done:
