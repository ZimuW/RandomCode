...
function getFilenames()
    queue = {}
    count = 1
    for file_folder in lfs.dir(input) do
	if file_folder ~= '.' and file_folder ~= '..' then
	    for file in lfs.dir(input .. file_folder) do
                if file ~= '.' and file ~= '..' then
		    --print (file_folder .."/" .. file)
            	    queue[count] = file_folder .. "/" .. file
            	    count = count + 1
                end
	    end
	end
...
function fillTensor(tensor)
  for i = 1, train_size do
    --print (input .. filenames[torch.random(1, #filenames)])
